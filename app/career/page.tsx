...
const CareersPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-[#213e6c]">
      <header className="py-10 text-center border-b border-gray-100">
        <h1 className="text-5xl font-bold text-[#213e6c]">Careers</h1>
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        <div className="mb-12">
          <span className="px-3 py-3 rounded-full border border-orange-400 text-orange-600 text-sm font-medium">
            We're Hiring
          </span>
          <h2 className="text-5xl font-extrabold text-[#213e6c] mt-6 mb-4 leading-tight">
        ...
        {categories.map((cat) => (
          <button
            ...
            className={`px-4 py-2 rounded-full border-2 transition-all font-bold ${
              activeCategory === cat 
              ? "bg-[#ed502e]  text-white border-[#ed502e]" 
       ...
       <hr className="border-t-2 border-[#213e6c] opacity-100 mb-0" />
        <section>
  ...
  filteredJobs.map((job, index) => (
    <div 
      key={job.id} 
      className={`py-8 ${
        index !== filteredJobs.length - 1 ? "border-b-2 border-[#213e6c]" : ""
      }`}
    >
      <div className="flex flex-col md:flex-row justify-between items-start gap-5">
        <div className="flex-1">
          <h3 className="font-['Luxurious Roman'] text-2xl font-bold text-[#213e6c] mb-3 tracking-tight">
            {job.title}
          </h3>
          ...
          <div className="flex gap-1">
            <span className="flex items-center gap-2 px-4 py-2 border-2 border-[#213e6c] rounded-full text-[#213e6c] font-medium text-sm">
        ...
        <a 
          href={`/apply/${job.id}`}
          className="flex items-center gap-2 font-bold text-2xl text-[#213e6c] hover:text-orange-500 transition-colors shrink-0"
        >
          Apply 
